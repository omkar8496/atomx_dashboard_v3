...
export default function EditAccessGateConfigModal({ gateConfig, onClose, onConfirm }) {
  const [name, setName] = useState(gateConfig?.gatesmaster_name ?? "");
  const [useCatgCounts, setUseCatgCounts] = useState(Number(gateConfig?.gatesmaster_useCatgCounts) === 1);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  useEffect(() => {
    setName(gateConfig?.gatesmaster_name ?? "");
    setUseCatgCounts(Number(gateConfig?.gatesmaster_useCatgCounts) === 1);
    setError("");
  }, [gateConfig]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError("Unable to update this gate configuration. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="edit-access-gate-config-title"
        ...
        >
          <div className="relative flex items-start justify-between gap-3">
            <div>
              ...
              <h2 id="edit-access-gate-config-title" className="font-chillax mt-1 text-[clamp(18px,2.2vw,24px)] font-semibold tracking-[-0.01em] text-white">
                Edit Gate Config
              </h2>
            </div>
            <button
              ...
              aria-label="Close gate configuration editor"
            >
              <CloseIcon />
            </button>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 max-[640px]:gap-3">
            ...
            <div>
              ...
              <div className="font-vcr flex h-11 items-center rounded-[10px] border border-(--line) bg-(--surface2) px-3.5 text-[13px] text-(--faint)">
                {gateConfig?.gatesmaster_position ?? "-"}
              </div>
            </div>
